...
Rule 1 makes the macro re-runnable: widen one value by hand, run it again,
and the columns tidy back up. Rule 2 is why it is worth running this BEFORE
Normalize Characters on a tab-separated file - Normalize turns each tab into
a single space, and at that point an empty field between two tabs is no
longer distinguishable from ordinary spacing. Either order works otherwise.
